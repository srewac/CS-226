...
public class DataFileReader {
    public ArrayList readFromTokenizedData(String datasetPath, String stopwordsPath) throws Exception {
        ...
        while(scanner.hasNextLine())
        {
            ...
            if(contents.length == 2 && contents[1].split(" ").length > 2)
            {
                String[] textWithTag = process(line, stopwordsPath);
                if(textWithTag[1].split(" ").length > 5)
    ...
    public ArrayList readRedditData(String datasetPath, String stopwordsPath) throws Exception {
        //HashMap<String, ArrayList<String>> dataMap = new HashMap<String, ArrayList<String>>();
        ArrayList<String> list = new ArrayList<String>();
        File dataFile = new File(datasetPath);
        if(!dataFile.exists())
        ...
        while(scanner.hasNextLine())
        {
            ...
            {
                String[] res = process(line, stopwordsPath);
                if(res[1].split(" ").length > 5)
                    list.add(res[0]+";"+res[1]);
            }
    ...
    public String[] process(String line, String stopwordsFilePath) throws Exception {
        ...
        for(String str : textArr)
        {
            //str = str.replace(" ","");
            int status = 0;
            if(!str.contains("@") && !str.contains("#") && !str.contains("https://") && !str.contains("http"))
            {
                status = 1;
            }
